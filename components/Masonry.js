...
const Masonry = ({
  ...
  const grid = useMemo(() => {
    ...
    const gridItems = items.map((child) => {
      ...
      const x = col * columnWidth + (col * gap / columns);
      const height = Math.max(200, child.height * 0.6); // Ensure minimum height and scale down
      const y = colHeights[col];

      colHeights[col] += height + gap;

      return { ...child, x, y, w: columnWidth - (gap / columns), h: height };
    });
  ...
  const containerHeight = useMemo(() => {
    ...
    return Math.max(600, maxHeight + 100);
  }, [grid]);
